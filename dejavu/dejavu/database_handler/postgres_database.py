...
class PostgreSQLDatabase(CommonDatabase):
    ...
    SELECT_RELATED_AUDIOS = f"""
        ...
        WHERE "{FIELD_RELATED_AUDIOS_AUDIO_ID}" = %(audio_id)s AND "{FIELD_RELATED_AUDIOS_FINGERPRINTED_CONFIDENCE}" >= %(fingerprinted_confidence)s
        ORDER BY "{FIELD_RELATED_AUDIOS_FINGERPRINTED_CONFIDENCE}" DESC;
    """

    SELECT_RELATED_AUDIOS_BYPRECISE = f"""
        SELECT
            "{FIELD_RELATED_AUDIOS_ID}"
        ,   "{FIELD_RELATED_AUDIOS_AUDIO_ID}"
        ,   "{FIELD_RELATED_AUDIOS_RELATED_AUDIO_ID}"
        ...
        AND "{FIELD_RELATED_AUDIOS_FINGERPRINTED_CONFIDENCE}" >= %s
        AND "{FIELD_RELATED_AUDIOS_RELATED_AUDIO_NAME}" LIKE '%%%%%s%%%%'
        ORDER BY "{FIELD_RELATED_AUDIOS_FINGERPRINTED_CONFIDENCE}" DESC;
    """
